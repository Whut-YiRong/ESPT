...
with open(os.path.join(origin_path,'mini-ImageNet.txt')) as f:
    lines = f.readlines()
    for line in lines:
...
for i in range(3):
    ...
    for index in tqdm(split_img[i]):
        ...
        origin_img = os.path.join(origin_path,'mini-ImageNet',img_path)
        target_img = os.path.join(split_path,img_path)
        shutil.copy(origin_img,target_img)

print('getting pre-resized 84x84 mini-ImageNet for validation and test')
